...
class embedder:
    def __init__(self, args):
        ...
        adj, features, labels, idx_train, idx_val, idx_test, idx_eval_train, idx_eval_val, idx_eval_test = process.load_data_dblp(args)
        ...
        self.idx_eval_train = torch.LongTensor(idx_eval_train).to(args.device)
        self.idx_eval_val = torch.LongTensor(idx_eval_val).to(args.device)
        self.idx_eval_test = torch.LongTensor(idx_eval_test).to(args.device)

        self.idx_train = [torch.LongTensor(i) for i in idx_train]
        self.idx_val = [torch.LongTensor(i) for i in idx_val]
        self.idx_test = [torch.LongTensor(i) for i in idx_test]
        ...
        for i in range(len(idx_train)):
            ...
            self.train_lbls = torch.argmax(self.labels[0, self.idx_train[i]], dim=1)
            self.val_lbls = torch.argmax(self.labels[0, self.idx_val[i]], dim=1)
            self.test_lbls = torch.argmax(self.labels[0, self.idx_test[i]], dim=1)
