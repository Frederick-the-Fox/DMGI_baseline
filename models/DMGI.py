...
class DMGI(embedder):
    ...
    def training(self):
        ...
        for epoch in range(self.args.nb_epochs):
            ...
            embeds = model.H.data
            metric = validate(embeds, 40, self.idx_eval_train, self.idx_eval_val, self.idx_eval_test, self.labels, nb_classes, self.args.device, self.args.dataset,
                self.args.eval_lr, self.args.eval_wd)

            print('now is the {}th epoch, the loss is :{}, the metric is:{}, best_m is :{}, metric.sum is :{}'.format(epoch, xent_loss, metric, best_m, metric.sum()))

            if metric.sum() > best_m:
                best_m = metric.sum()
                best_t = epoch
                cnt_wait = 0
                torch.save(model.state_dict(), 'DMGI_' + self.args.dataset + '.pkl')
            else:
                cnt_wait += 1
            ...
            print('the {}th epoch done. time is {} seconds, cnt_wait is :{}'.format(epoch, end_time - start_time, cnt_wait))
            time.append(end_time - start_time)

        print('train process done. the mean time is:{}seconds.'.format(np.mean(time)))

        model.load_state_dict(torch.load('DMGI_' + self.args.dataset + '.pkl'))

        # Evaluation
        model.eval()
        ratio = [20, 40, 60]
        for i in range(len(self.idx_train)):
            evaluate(model.H.data.detach(), self.idx_train[i], self.idx_val[i], self.idx_test[i], self.labels, self.args.device, ratio[i], 0.01, 0)
