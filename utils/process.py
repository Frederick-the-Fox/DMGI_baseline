...
def load_data_dblp(args):
    ...
    idx_eval_train = data['train_idx_eval'].ravel()
    idx_eval_val = data['val_idx_eval'].ravel()
    idx_eval_test = data['test_idx_eval'].ravel()

    truefeatures_list = []
    for _ in range(len(rownetworks)):
        truefeatures_list.append(truefeatures)

    return rownetworks, truefeatures_list, label, idx_train, idx_val, idx_test, idx_eval_train, idx_eval_val, idx_eval_test
